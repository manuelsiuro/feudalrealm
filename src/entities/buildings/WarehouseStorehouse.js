...
class WarehouseStorehouse extends Building {
    constructor(gridX, gridZ, gameMap) {
        const buildingDataEntry = BUILDING_DATA.WAREHOUSE_STOREHOUSE;
    ...
    createModel() {
        ...
        const buildingWidth = TILE_SIZE * 2.0;
        const buildingHeight = TILE_SIZE * 1.0;
        const buildingDepth = TILE_SIZE * 1.25;
        ...
        const roofWidth = buildingWidth + TILE_SIZE * 0.1;
        const roofSlopeHeight = TILE_SIZE * 0.3; // Height of the roof peak from building top
        const roofPanelDepth = (buildingDepth / 2) + TILE_SIZE * 0.05; // Depth of one slope panel
        ...
        const doorWidth = TILE_SIZE * 0.4;
        const doorHeight = buildingHeight * 0.6;
        const doorDepth = TILE_SIZE * 0.05; // Indentation depth
        const doorMaterial = new THREE.MeshPhongMaterial({ color: 0x5C3317 }); // Darker than building
        
        const doorPositions = [
            // Front doors
            new THREE.Vector3(-buildingWidth/2 + doorWidth/2 + TILE_SIZE * 0.15, doorHeight/2, buildingDepth/2 - doorDepth/2 + 0.01),
            new THREE.Vector3(TILE_SIZE * 0.0, doorHeight/2, buildingDepth/2 - doorDepth/2 + 0.01),
            new THREE.Vector3(buildingWidth/2 - doorWidth/2 - TILE_SIZE * 0.15, doorHeight/2, buildingDepth/2 - doorDepth/2 + 0.01),
