...
class Marketplace extends Building {
    constructor(gridX, gridZ, gameMap) {
        const buildingDataEntry = BUILDING_DATA.MARKETPLACE;
    ...
    createModel() {
        ...
        const centralWidth = TILE_SIZE * 1.25;
        const centralPostHeight = TILE_SIZE * 0.5; 
        const centralDepth = TILE_SIZE * 1.25;
        
        // Posts for central area
        const postSize = TILE_SIZE * 0.1;
        ...
        const roofHeight = TILE_SIZE * 0.5;
        ...
        const stallWidth = TILE_SIZE * 0.4;
        const stallBaseHeight = TILE_SIZE * 0.35;
        const stallDepth = TILE_SIZE * 0.3;
        const stallRoofHeight = TILE_SIZE * 0.05;

        const stallData = [
            { x: centralWidth/2 + stallWidth/2 + TILE_SIZE * 0.1, z: 0, color: 0xFF6347 /* Tomato */ },
            { x: -centralWidth/2 - stallWidth/2 - TILE_SIZE * 0.1, z: 0, color: 0x4682B4 /* SteelBlue */ },
            { x: 0, z: centralDepth/2 + stallDepth/2 + TILE_SIZE * 0.1, color: 0x3CB371 /* MediumSeaGreen */ },
            { x: 0, z: -centralDepth/2 - stallDepth/2 - TILE_SIZE * 0.1, color: 0xEE82EE /* Violet */ },
        ];
        ...
        stallData.forEach(sp => {
            ...
            const stallRoofMesh = new THREE.Mesh(
                new THREE.BoxGeometry(stallWidth + TILE_SIZE * 0.05, stallRoofHeight, stallDepth + TILE_SIZE * 0.05),
