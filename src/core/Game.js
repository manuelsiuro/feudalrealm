...
import ResourceFlowManager from './ResourceFlowManager.js';
import ProductionChainManager from './ProductionChainManager.js';
...
class Game {
    constructor() {
        ...
        this.natureManager = null;
        this.resourceManager = resourceManager;
        this.resourceFlowManager = null;
        this.productionChainManager = null;
    ...
    init() {
        ...
        console.log(
            'GameMap created, features generated, and added to scene via Game.js.'
        );
        ...
        this.renderer.gameElementsGroup.add(
            this.natureManager.resourceNodesGroup
        );
        console.log(
            'NatureManager initialized and its resources added to scene.'
        );
        ...
        const groundMaterial = new THREE.MeshBasicMaterial({
            color: TERRAIN_COLORS[TERRAIN_TYPES.GRASSLAND],
        });
        ...
        //  will be given SerfManager instance later to avoid circular dependency at construction
        this.constructionManager = new ConstructionManager(
            this.scene,
            this.gameMap,
            this.renderer.gameElementsGroup
        );
        ...
        this.constructionManager.setResourceFlowManager(
            this.resourceFlowManager
        );

        // Initialize ProductionChainManager
        this.productionChainManager = new ProductionChainManager(
            this.gameMap,
            this.resourceManager
        );
        console.log(
            'ProductionChainManager initialized for production chains.'
        );
        ...
        this.serfManager = new SerfManager(
            this.scene,
            this.gameMap,
        ...
        this.constructionManager.setSerfManager(this.serfManager);
        ...
        // Configure a SelectionManager with selectable groups
        const buildingsGroup =
            this.renderer.gameElementsGroup.getObjectByName('GameBuildings');
        ...
        this.uiManager.setBuildingSelectCallback((buildingModelId) => {
        ...
        // 7. Set up New Event Handlers using InputManager and SelectionManager
        ...
        this.uiManager.updateResourceUI(
            this.resourceManager.getAllStockpiles()
        );
    ...
    setupCameraAndControls() {
        ...
        this.controls.maxDistance =
            Math.max(MAP_WIDTH, MAP_HEIGHT) * TILE_SIZE * 2.5;
        this.controls.minDistance = TILE_SIZE * 2;
    }

    setupNewEventHandlers() {
        const tempRaycaster = new THREE.Raycaster(); // For placement logic, Game still does this ray-cast

        this.inputManager.onClick((clickData) => {
            // Prevent click handling if a UI element was the target
            if (
                clickData.rawEvent.target.closest('.themed-panel') ||
                clickData.rawEvent.target.closest('.ui-button')
            ) {
                console.log(
                    'Game: Click on UI element ignored for game world interaction.'
                );
                return;
            }

            if (this.constructionManager.isPlacing) {
                tempRaycaster.setFromCamera(
                    { x: clickData.normalizedX, y: clickData.normalizedY },
                    this.camera
                );
                const intersectsPlacement = tempRaycaster.intersectObject(
                    this.gameMap.tileMeshes,
                    true
                );
                ...
                if (intersectsPlacement.length > 0) {
                    ...
                    tempRaycaster.ray.intersectPlane(
                        groundPlane,
                        placementPoint
                    );
                }
                ...
                if (placementPoint) {
                    const success =
                        this.constructionManager.confirmPlacement(
                            placementPoint
                        );
                    if (success) {
                        // Update selectable groups in the SelectionManager if a new building was added
                        const buildingsGroup =
                            this.renderer.gameElementsGroup.getObjectByName(
                                'GameBuildings'
                            );
                        const serfsGroup = this.serfManager.serfVisualsGroup;
                        this.selectionManager.setSelectableGroups(
                            buildingsGroup,
                            serfsGroup
                        );
                    }
                ...
                this.selectionManager.handleSelectionClick({
        ...
        this.inputManager.onMouseMove((moveData) => {
            if (this.constructionManager.isPlacing) {
                tempRaycaster.setFromCamera(
                    { x: moveData.normalizedX, y: moveData.normalizedY },
                    this.camera
                );
                const intersects = tempRaycaster.intersectObject(
                    this.gameMap.tileMeshes,
                    true
                );
                const groundPlane = new THREE.Plane(
                ...
                if (indicatorPoint) {
                    this.constructionManager.updatePlacementIndicator(
                        indicatorPoint
                    );
                }
            }
        ...
        this.inputManager.onKeyDown((event) => {
            ...
            if (event.key.toLowerCase() === 'f') {
                if (event.shiftKey) {
                    ...
                    console.log(
                        `🎨 Flow particles ${this.resourceFlowManager.config.showParticles ? 'enabled' : 'disabled'}`
                    );
                } else {
                    // F: Toggle flows
                    this.resourceFlowManager.toggleFlows();
                    console.log(
                        `🌊 Resource flows ${this.resourceFlowManager.config.showFlows ? 'enabled' : 'disabled'}`
                    );
                }
            }
            if (event.key.toLowerCase() === 't') {
                ...
                console.log(
                    `👣 Serf trails ${this.resourceFlowManager.config.showTrails ? 'enabled' : 'disabled'}`
                );
            }
        });
        ...
        this.selectionManager.onSelectionChange((selectedEntity) => {
            if (selectedEntity && selectedEntity.model) {
                ...
                // Enhanced selection feedback based on an entity type
                if (selectedEntity.info && selectedEntity.info.name) {
                    console.log(
                        `🏗️ Building Selected: ${selectedEntity.info.name} ${selectedEntity.isConstructed ? '(Complete)' : '(Under Construction)'}`
                    );
                } else if (selectedEntity.serfType) {
                    console.log(
                        `👷 Serf Selected: ${selectedEntity.serfType} (ID: ${selectedEntity.id})`
                    );
                }
    ...
    selectAndFocusBuilding(buildingModelId) {
        // Parameter is model UUID
        ...
        const building = allBuildings.find(
            (b) => b.model && b.model.uuid === buildingModelId
        );
        ...
        if (building) {
            ...
            console.warn(
                `Game: Building with model ID ${buildingModelId} not found.`
            );
    ...
    start() {
        this.animate();
        // Ensure the UI is correctly sized after all initializations
        this.handleResize();
    }
    ...
    animate() {
        ...
        if (this.constructionManager) {
            ...
            if (this.constructionManager.constructionEffectsManager) {
                this.constructionManager.constructionEffectsManager.update(
                    deltaTime
                );
            }
        }
