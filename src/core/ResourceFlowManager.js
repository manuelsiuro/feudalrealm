...
export class ResourceFlowManager {
    ...
    recordFlow(fromBuildingId, toBuildingId, resourceType, amount) {
        ...
        if (!fromBuilding || !toBuilding) {
            console.warn(
                `ResourceFlowManager: Could not find buildings for flow ${flowId}`
            );
            return;
        }
        ...
        const flowData = {
            ...
            totalVolume:
                (this.activeFlows.get(flowId)?.totalVolume || 0) + amount,
        ...
        console.log(
            `📈 Flow recorded: ${amount} ${resourceType} from ${fromBuilding.info?.name || fromBuilding.type} to ${toBuilding.info?.name || toBuilding.type}`
        );
    }
    ...
    createOrUpdateFlowLine(flowData) {
        ...
        const fromPos =
            fromBuilding.model?.position ||
            fromBuilding.position ||
            new THREE.Vector3();
        const toPos =
            toBuilding.model?.position ||
            toBuilding.position ||
            new THREE.Vector3();
        ...
        const color =
            this.resourceColors[resourceType] || this.resourceColors.default;
    ...
    createFlowParticles(flowData) {
        ...
        const particleCount = Math.min(
            10,
            Math.max(3, Math.floor(flowData.frequency * 2))
        );
        ...
        const color = new THREE.Color(
            this.resourceColors[flowData.resourceType] ||
                this.resourceColors.default
        );
    ...
    updateParticles(deltaTime) {
        this.particleSystems.forEach((particles, flowId) => {
            ...
            for (let i = 0; i < progress.count; i++) {
                ...
                const point = particles.userData.curve.getPoint(
                    progress.array[i]
                );
    ...
    updateSerfTrailVisual(trail) {
        ...
        if (trail.visualTrail) {
            ...
            if (trail.visualTrail.geometry)
                trail.visualTrail.geometry.dispose();
            if (trail.visualTrail.material)
                trail.visualTrail.material.dispose();
        }
    ...
    updateFlowVisualization(deltaTime) {
        ...
        this.flowLines.forEach((flowLineData, flowId) => {
            ...
            const timeSinceLastActivity =
                Date.now() - flowLineData.flowData.lastActivity;
            const pulseIntensity = Math.max(
                0.3,
                1.0 - timeSinceLastActivity / 10000
            ...
            material.opacity =
                ...
                0.3 *
                    pulseIntensity *
                    (0.8 + 0.2 * Math.sin(Date.now() * 0.003));
        });
    }
        ...
        this.serfTrails.forEach((trail, serfId) => {
            if (trail.timestamps.length > 0) {
                ...
                if (now - oldestTimestamp > maxAge) {
                    ...
                    while (
                        trail.timestamps.length > 0 &&
                        now - trail.timestamps[0] > maxAge
                    ) {
                        trail.positions.shift();
                        trail.resources.shift();
                        trail.timestamps.shift();
    ...
    findBuildingById(buildingId) {
        return this.constructionManager.placedBuildings.find(
            (building) =>
    ...
    toggleFlows() {
        ...
        console.log(
            `🔄 Flow visualization: ${this.config.showFlows ? 'ON' : 'OFF'}`
        );
    }
    ...
    toggleParticles() {
        ...
        console.log(
            `🔄 Flow particles: ${this.config.showParticles ? 'ON' : 'OFF'}`
        );
    }
