...
import {
    BUILDING_STATE_NEEDS_CONSTRUCTION,
    BUILDING_STATE_UNDER_CONSTRUCTION,
    BUILDING_STATE_CONSTRUCTED,
} from '../../entities/Building.js'; // Added import
...
class ConstructBuildingTask extends Task {
     ...
     */
    constructor(buildingInstance, scene, priority) {
    ...
    onAssign(serf) {
        super.onAssign(serf);
        console.log(
            `[ConstructBuildingTask ON_ASSIGN] Task ${this.id} for ${this.building.name} (ID: ${this.building.id}) assigned to Serf ${serf.id}. Building state: ${this.building.currentConstructionState}`
        );
        ...
        if (
            this.building.currentConstructionState ===
            BUILDING_STATE_NEEDS_CONSTRUCTION
        ) {
            console.log(
                `[ConstructBuildingTask ON_ASSIGN] Calling startConstructionProcess for ${this.building.name}`
            );
            ...
            console.log(
                `[ConstructBuildingTask ON_ASSIGN] Building ${this.building.name} was already UNDER_CONSTRUCTION, re-assigning builder ${serf.id}`
            );
        } else if (
            this.building.currentConstructionState ===
                BUILDING_STATE_UNDER_CONSTRUCTION &&
            ...
            console.warn(
                `[ConstructBuildingTask ON_ASSIGN] Building ${this.building.name} is already UNDER_CONSTRUCTION by another builder (${this.building.assignedBuilderId}). Serf ${serf.id} cannot take over. Task should fail or be re-evaluated.`
            );
        ...
        console.log(
            `[ConstructBuildingTask ON_ASSIGN] Commanding serf ${serf.id} to move to ${entryPoint.x}, ${entryPoint.z} for building ${this.building.name}`
        );
        this._moveToLocation(
            serf,
            { x: entryPoint.x, y: entryPoint.z },
            SERF_ACTION_STATES.CONSTRUCTING_BUILDING
        );
    }
    ...
    onUpdate(serf, deltaTime) {
        ...
        if (this.isComplete(serf)) {
            // Checks building.currentConstructionState
            console.log(
                `[ConstructBuildingTask ON_UPDATE] Task for ${this.building.name} is complete. Calling onComplete.`
            );
            ...
            console.log(
                `[ConstructBuildingTask ON_UPDATE] Task ${this.id}: Serf ${serf.id} is IDLE, re-engaging for construction of ${this.building.name}`
            );
            ...
            if (
                ...
                if (
                    ...
                    console.log(
                        `[ConstructBuildingTask ON_UPDATE] Re-engaging: Calling startConstructionProcess for ${this.building.name}`
                    );
                    this.building.startConstructionProcess(serf.id, this.scene);
                } else {
                    this.building.assignedBuilderId = serf.id;
                    console.log(
                        `[ConstructBuildingTask ON_UPDATE] Re-engaging: Re-assigned builder ${serf.id} to ${this.building.name}`
                    );
                }
            } else if (
                this.building.currentConstructionState ===
                ...
                console.warn(
                    `[ConstructBuildingTask ON_UPDATE] Task ${this.id}: Cannot re-engage serf ${serf.id}. Building ${this.building.name} assigned to ${this.building.assignedBuilderId}. Failing task.`
                );
                this.onFail(serf);
                return;
            }
            ...
            const entryPoint = this.building.getEntryPointGridPosition();
            console.log(
                `[ConstructBuildingTask ON_UPDATE] Re-engaging: Commanding serf ${serf.id} to move to ${entryPoint.x}, ${entryPoint.z} for building ${this.building.name}`
            );
            this._moveToLocation(
                serf,
                { x: entryPoint.x, y: entryPoint.z },
    ...
    onComplete(serf) {
        ...
        console.log(
            `[ConstructBuildingTask ON_COMPLETE] Task for ${this.building.name} (${this.building.id}) completed by Serf ${serf.id}. Serf state: ${serf.currentState.name}`
        );
    }
    ...
    onFail(serf) {
        ...
        console.log(
            `[ConstructBuildingTask ON_FAIL] Task for ${this.building.name} failed. Serf: ${serf ? serf.id : 'none'}`
        );
        ...
        if (
            ...
            // Added null check for serf
            console.log(
                `[ConstructBuildingTask ON_FAIL] Clearing assigned builder ${serf.id} from ${this.building.name}.`
            );
            this.building.assignedBuilderId = null;
            ...
            if (
                ...
                console.log(
                    `[ConstructBuildingTask ON_FAIL] Building ${this.building.name} was UNDER_CONSTRUCTION. ConstructionManager should requeue it.`
                );
            }
        }
     ...
     */
    onCancel(serf) {
        // Added serf parameter, was missing
        ...
        if (
            this.building &&
            this.assignedSerf &&
            this.building.assignedBuilderId === this.assignedSerf.id
        ) {
            console.log(
                `[ConstructBuildingTask ON_CANCEL] Task for ${this.building.name} cancelled. Clearing assigned builder ${this.assignedSerf.id}.`
            );
            this.building.assignedBuilderId = null;
            if (
                this.building.currentConstructionState ===
                ...
                console.log(
                    `[ConstructBuildingTask ON_CANCEL] Building ${this.building.name} was UNDER_CONSTRUCTION. ConstructionManager should requeue it.`
                );
            }
        }
        ...
        if (
            this.assignedSerf &&
            this.assignedSerf.currentState.name !== SERF_ACTION_STATES.IDLE
        ) {
